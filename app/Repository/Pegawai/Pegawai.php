...
class Pegawai
{
    public function getPegawai()
    {
        return DB::connection('sqlsrv_simrs')->table('pegawai as p')->select(
                'p.kd_pegawai', 'p.nama_pegawai', 'p.alamat', 'p.nip', 'p.gelar_depan', 'p.gelar_belakang', 'p.tgl_lahir',
                'su.nama_sub_unit'
            )
    ...
    public function getPegawaiDetail($kodePegawai)
    {
        ...
        return DB::connection('sqlsrv_sms')
            ...
            ->select('a.kd_pegawai','a.mac_address', 'device', 'a.status_update','a.created_at','a.updated_at',
                    'p.nama_pegawai','p.gelar_depan','p.gelar_belakang','p.foto','su.kd_sub_unit','tempat_lahir','tgl_lahir',
                    'su.nama_sub_unit')
